...
public class BLUStrike extends BaseCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        List<AbstractCard> jobStrikeCards = new ArrayList<>();
        for (AbstractCard card : CardLibrary.getAllCards()) {
            if (card.hasTag(CustomTags.FFJOBSTRIKE)) {
                if (upgraded) {card.upgrade();}
                jobStrikeCards.add(card);
            }
        }
        CardGroup addingCardGroup = new CardGroup(CardGroup.CardGroupType.UNSPECIFIED);
        Collections.shuffle(jobStrikeCards);
        int maxCards = Math.min(magicNumber, jobStrikeCards.size());

        for (int i = 0; i < maxCards; i++) {
            addingCardGroup.addToTop(jobStrikeCards.get(i));
        }

        if (!addingCardGroup.isEmpty()) {
            AbstractDungeon.gridSelectScreen.open(addingCardGroup, 1, "Choose Attack Mimicry.", false, false, false, false);
        }
