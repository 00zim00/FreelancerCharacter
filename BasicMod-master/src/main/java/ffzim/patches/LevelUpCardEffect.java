...
public class LevelUpCardEffect extends AbstractGameEffect {
    private AbstractCard card;
    private static final float PADDING;
    private float scaleY;
    private Color rarityColor;

    public LevelUpCardEffect(AbstractCard card) {
        this(card, (float)Settings.WIDTH - 96.0F * Settings.scale, (float)Settings.HEIGHT - 32.0F * Settings.scale);
    }

    public LevelUpCardEffect(AbstractCard card, float x, float y) {
        this.card = card;
    ...
    private void identifySpawnLocation(float x, float y) {
        ...
        while(var4.hasNext()) {
            ...
            if (e instanceof com.megacrit.cardcrawl.vfx.cardManip.PurgeCardEffect) {
                ++effectCount;
            }
        }
