...
public class StealActionRedo extends AbstractGameAction {
    private static final float DUR = 0.25F;
    private int itemAmount;
    private int rngRoll;
    private String itemType;
    private String itemName;
    public StealActionRedo(AbstractCreature target, AbstractCreature source,int rngRoll, int itemAmount, String itemType, String itemName) {
        setValues(target, source);
        this.target = target;
        this.source = source;
        this.rngRoll = rngRoll;
    ...
    public void update() {
        if (this.target.hasPower(makeID("StealTaggingPower"))){
            if (Objects.equals(this.itemType, "Gold")){
                stealGold();
            }
            if (Objects.equals(this.itemType, "Potion")){

            }
            if (Objects.equals(this.itemType, "ItemCard")){

            }
            if (Objects.equals(this.itemType, "Card")){

            }
            if (Objects.equals(this.itemType, "Relic")){

            }
            if (Objects.equals(this.itemType, "BlueMagic")){
