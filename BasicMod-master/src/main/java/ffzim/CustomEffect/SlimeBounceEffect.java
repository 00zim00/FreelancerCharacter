...
public class SlimeBounceEffect extends AbstractGameEffect {
    ...
    public SlimeBounceEffect(float srcX, float srcY, float destX, float destY) {
        if (img == null)
    ...
    public void render(SpriteBatch sb) {
        ...
        for (int i = 5; i < this.previousPos.size(); i++)
            sb.draw((TextureRegion)ImageMaster.POWER_UP_2, ((Vector2)this.previousPos
                    ...
                    .get(i)).y - (img.packedHeight / 2), img.packedWidth / 2.0F, img.packedHeight / 2.0F, img.packedWidth, img.packedHeight, this.scale / 40.0F / i, this.scale / 40.0F / i, this.rotation);
        sb.setColor(this.color);
        sb.draw((TextureRegion)img, this.cX - (img.packedWidth / 2), this.cY - (img.packedHeight / 2) + this.yOffset, img.packedWidth / 2.0F, img.packedHeight / 2.0F, img.packedWidth, img.packedHeight, this.scale, this.scale, this.rotation);
        sb.draw((TextureRegion)img, this.cX - (img.packedWidth / 2), this.cY - (img.packedHeight / 2) + this.yOffset, img.packedWidth / 2.0F, img.packedHeight / 2.0F, img.packedWidth, img.packedHeight, this.scale, this.scale, this.rotation);
